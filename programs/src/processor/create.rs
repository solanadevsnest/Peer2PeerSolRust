...
pub fn launch_trade_workflow<'info>(
    ...
    let vault_marker = params.vault_marker;
    ...
    let trade_type: TradeType = match creator_accounts {
        ...
        (Err(_), Ok(creator_receive_account)) => match mints_address {
            (Some(Ok(mint_token_creator_receive)), None) => {
                ...
                ctx.accounts
                    .create_native_account_vault(vault_marker, params.order_id)?;
    ...
    ctx.accounts.escrow_state.vault_marker = vault_marker;
...
impl<'info> Create<'info> {
    ...
    fn create_native_account_vault(&self, vault_marker: u8, order_id: u64) -> Result<()> {
        let creator_key = self.creator.key();
        let order_id_bytes = order_id.to_le_bytes();
        let vault_seed = &[&[
            VAULT_PDA_SEED,
