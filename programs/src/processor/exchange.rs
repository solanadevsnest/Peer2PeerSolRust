...
#[derive(Accounts)]
#[instruction(order_id: u64, state_bump: u8, vault_marker: u8)]
pub struct Exchange<'info> {
    ...
    #[account(mut)]
    pub counterparty_sender: AccountInfo<'info>,
...
pub fn handler_exchange(ctx: Context<Exchange>) -> Result<()> {
    ...
    match trade_type {
        ...
        Ok(TradeType::TokenToken) => {
            // convert `counterparty_sender` to TokenAccount
            let partner_token_accounts: (TokenAccountType, TokenAccountType) = (
                Account::try_from(&ctx.accounts.counterparty_sender),
            ...
            match partner_token_accounts {
                (Ok(partner_send_token_account), Ok(partner_receive_token_account)) => {
                    ...
                    // make sure `counterparty_sender` is associated token account with `creator_receive_token_mint`
        ...
        Ok(TradeType::TokenSol) => {
            ...
            ctx.accounts.transfer_native_to_creator(receive_value)?;
            ctx.accounts.end_token_vault()?;
        }
        // Case SOL - SPL
...
impl<'info> Exchange<'info> {
    fn transfer_to_partner_native(&self, amount: u64) -> Result<()> {
        ...
        let vault_marker = self.escrow_state.vault_marker;
        let vault_seed = &[&[
            VAULT_PDA_SEED,
            creator_key.as_ref(),
            order_id_bytes.as_ref(),
            bytemuck::bytes_of(&vault_marker),
        ][..]];
    ...
    fn transfer_native_to_creator(&self, amount: u64) -> Result<()> {
        // transfer SOL partner -> creator
        transfer_native_to_account(
            self.partner.to_account_info(),
    ...
    fn transfer_to_creator_token(&self, amount: u64) -> Result<()> {
        // transfer token partner -> creator
        transfer_token_to_account(
            self.counterparty_sender.to_account_info(),
    ...
    fn end_token_vault(&self) -> Result<()> {
